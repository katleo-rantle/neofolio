interface RobotSVGProps {
  state: 'idle' | 'hover' | 'typing' | 'responding' | 'user-typing';
}
export function RobotSVG({ state }: RobotSVGProps) {
  const isActive =
    state === 'typing' || state === 'responding' || state === 'user-typing';
  const glowOpacity =
    state === 'user-typing' ? '0.5' : state === 'hover' ? '0.3' : '0';
  const bodyAnim =
    state === 'hover' || state === 'user-typing'
      ? 'scale-110'
      : state === 'typing'
        ? 'animate-bounce'
        : '';
  const earColor = isActive ? 'var(--hud-accent)' : '#EA5D5C';
  const visorColor = isActive ? 'var(--hud-primary)' : '#4C66AF';
  const visorClass =
    state === 'responding'
      ? 'animate-pulse'
      : state === 'user-typing'
  ...
  return (
    <svg
      ...
      className={`w-full h-full transition-transform duration-300 ${bodyAnim}`}
    ...
    >
      ...
      {/* Ears */}
      <path
        d='M1071.786667 712.798161h72.503908v136.297931h-72.503908z'
        fill={earColor}
        className={`transition-colors duration-300 ${state === 'responding' ? 'animate-pulse' : ''}`}
      />

      <path
        d='M36.016552 712.798161h72.503908v136.297931H36.016552z'
        fill={earColor}
        className={`transition-colors duration-300 ${state === 'responding' ? 'animate-pulse' : ''}`}
      />
      ...
      {/* Visor / Screen */}
      <path
        d='M 321.019 574.671 L 847.305 573.71 L 847.68 730.635 L 321.396 731.596 L 321.019 574.671 Z'
        fill={visorColor}
        className={`transition-colors duration-300 ${visorClass}`}
        style={
          state === 'user-typing'
            ? {
                filter: `drop-shadow(0 0 15px ${visorColor})`,
              }
            : {}
        }
      />

      {/* Main outline and details */}
      <path
      ...
      <path
        ...
        fill='var(--hud-text)'
        className={`transition-colors duration-300 ${state === 'typing' || state === 'user-typing' ? 'animate-pulse' : ''}`}
        style={
          isActive
            ? {
                fill: 'var(--hud-primary)',
