...
import { navItems, type NavigationProps } from '../utils/types';
import { MobileNavFab } from './MobileNav';
...
export function Navigation({
  ...
  // 2. Updated Nav Click Handler
  const handleNavClickAction = (
  ...
  // 1. Intersection Observer to detect scroll position
  useEffect(() => {
  ...
  return (
    <>
          ...
          >
            <div className='flex items-center gap-3 group cursor-pointer'>
              ...
              <span
                ...
                data-text='KATLEO.DEV'
              >
                KATLEO<span className='text-hud-primary'>.DEV</span>
              </span>
            </div>

            {/* 3. Update Desktop Navigation Links with Active State */}
            <nav
              ...
              {navItems.map((link) => {
                ...
                return (
                  <a
                    ...
                    onClick={(e) => handleNavClickAction(e, item)}
                    className={`group relative flex items-center gap-1.5 text-hud-text-muted hover:text-hud-primary  hover:font-semibold transition-all duration-300 ${
                      isActive
                        ? 'text-hud-primary'
                        : 'text-hud-text-muted hover:text-hud-primary'
                    }`}
                  >
                    <span
                      className={`text-[8px] transition-colors ${
                        isActive
                          ? 'text-hud-accent scale-125 shadow-[0_10px_20px_var(--hud-accent)] animate-pulse'
                          : 'text-hud-primary/50 group-hover:text-hud-accent group-hover:scale-125'
                      }`}
                    >
                      ◆
                    </span>
                    ...
                    <div
                      className={`absolute -bottom-1 left-0 h-[1px] bg-hud-primary transition-all duration-300 ${
      ...
      <MobileNavFab
        ...
        activeSection={activeSection} // pass the string state here
      />
    </>
  );
}
